...
#[get("{server_id}")]
async fn server(request: HttpRequest, path: web::Path<u64>) -> Result<HttpResponse> {
	...
	let server = CACHE
		.discord
		.guild(server_id)
		.ok_or(ErrorModelKind::not_found(ResourceKind::Server, Some(server_id)))?;
...
async fn server_member_link_update(request: HttpRequest, path: web::Path<(u64, u64)>, payload: web::Json<UpdateMemberLink>) -> Result<HttpResponse> {
	...
	let pinned = Pin::static_ref(&PG_POOL)
		.await;
